 ...
 * An automatic column ID is created for all table<br>
 * <br>
...
public class Database
{
   ...
   private static final ColumnDescription COLUMN_PASSWORD    = new ColumnDescription(Database.PASSWORD, DataType.LONGVARCHAR);
   /** Column name for store password */
   private static final String            PASSWORD           = "password";

   /** Password table name */
   private static final String            PASSWORD_TABLE     = UtilText.concatenate(Database.class.getName(), "_PASSWORD_TABLE").replace('.', '_');

   /** Description of column of column name */
   public static final ColumnDescription  COLUMN_COLUMN_NAME = new ColumnDescription(Database.COLUMN_NAME, DataType.VARCHAR);

   /** Description of column of column type */
   public static final ColumnDescription  COLUMN_COLUMN_TYPE = new ColumnDescription(Database.COLUMN_TYPE, DataType.VARCHAR);
   ...
   public static final ColumnDescription  COLUMN_TABLE_NAME  = new ColumnDescription(Database.TABLE_NAME, DataType.VARCHAR);
   ...
   public static final String             METADATA_COLUMN    = UtilText.concatenate(Database.class.getName(), "_METADATA_COLUMN").replace('.', '_');

   /** Metadata table for store table name */
   public static final String             METADATA_TABLE     = UtilText.concatenate(Database.class.getName(), "_METADATA_TABLE").replace('.', '_');
   /**
    ...
    *           Database path
    * @param password
    *           Password to access encrypted data.
    * @throws DatabaseException
    *            On creation problem
    */
         ...
         throws DatabaseException
   {
      if(password == null)
      {
         throw new NullPointerException("password musn't be null");
      }
    ...
    *           Table where delete
    * @param where
    *           Condition that match deleting rows (Can be {@code null} for clear all rows)
    * @throws DatabaseException
    *            On deletion problem
    */
   ...
   public TableInfo getTableInfo(final String tableName) throws DatabaseException
   {
      ...
      selectQuery = new SelectQuery(Database.METADATA_COLUMN, Database.COLUMN_COLUMN_NAME, Database.COLUMN_COLUMN_TYPE);
    ...
    * 
    * @param table
    *           Table where insertion/update append
    * @param where
    *           Condition to know if it is insertion or update (Can be {@code null} to force an insert
    * @param values
    ...
    *           Query to execute
    * @param columnSort
    *           Column for sort (Can be {@code null} if no sort)
    * @param isAscendent
    *           Indicates if the sort is ascendent ({@code true}) or descendant ({@code false})
    * @return Result of query
    * @throws DatabaseException
    *            On execution problem
    */
   public QueryResult query(final SelectQuery query, final String columnSort, final boolean isAscendent) throws DatabaseException
   {
      try
      {
         // create the query
   ...
   public void update(final UpdateQuery query) throws DatabaseException
   {
      if(query == null)
      {
         throw new NullPointerException("query musn't be null");
      }
